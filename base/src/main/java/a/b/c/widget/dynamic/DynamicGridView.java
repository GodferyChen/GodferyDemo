...
public class DynamicGridView extends GridView {
    ...
    private void startWobbleAnimation() {
        for (int i = 0; i < getChildCount(); i++) {
            View v = getChildAt(i);
            if (v != null && Boolean.TRUE != v.getTag(R.id.dynamicGridItem)) {
                if (i % 2 == 0)
                    animateWobble(v);
                else
                    animateWobbleInverse(v);
                v.setTag(R.id.dynamicGridItem, true);
            }
        }
    }
    ...
    private void stopWobble(boolean resetRotation) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (v != null) {
                ...
                v.setTag(R.id.dynamicGridItem, false);
            }
        }
    }
    ...
    public void init(Context context) {
        ...
        mOverlapIfSwitchStraightLine = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 16 , getResources().getDisplayMetrics());
    }
    ...
    private OnScrollListener mScrollListener = new OnScrollListener() {
        ...
        private void updateWobbleState(int visibleItemCount) {
            for (int i = 0; i < visibleItemCount; i++) {
                ...
                if (child != null) {
                    if (mMobileItemId != INVALID_ID && Boolean.TRUE != child.getTag(R.id.dynamicGridItem)) {
                        if (i % 2 == 0)
                            animateWobble(child);
                        else
                            animateWobbleInverse(child);
                        child.setTag(R.id.dynamicGridItem, true);
                    } else if (mMobileItemId == INVALID_ID && child.getRotation() != 0) {
                        child.setRotation(0);
                        child.setTag(R.id.dynamicGridItem, false);
                    }
                }
